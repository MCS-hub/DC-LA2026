...
def init(args):
    ...
    global fwd_op_mod, fbp_op_mod, fwd_op_norm, fwd_op_adj_mod
    global size, angles, noise, rec_size,detectors
    ...
    size = 512
    angles = 350
    max_ang=2*np.pi/3 # missing  60 degrees
    ...
    detector_partition = odl.uniform_partition(-362.0, 362.0, args.detectors) # 700 rays/angle as specified, args.detectors=700
    geometry = odl.tomo.Parallel2dGeometry(angle_partition,detector_partition)
    # geometry = odl.tomo.parallel_beam_geometry(space, num_angles=angles, det_shape=(detectors,))
    fwd_op = odl.tomo.RayTransform(space, geometry, impl='astra_cuda') #TODO: change to astra_cuda
    ...
    fwd_op_norm = odl.power_method_opnorm(fwd_op, maxiter=100)
